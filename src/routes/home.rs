...
use yew::{Component, ComponentLink, Html, ShouldRender, html, web_sys::{FileSystemDirectoryHandle, window}};
...
async fn pick_directory() -> Result<String, JsFutureError> {
		...
		match msg {
			HomeMsg::ShowDirectoryPicker => {
				self.link.send_future(async {
					if let Ok(d) = pick_directory().await {
						debug!("{}", d);
					}
